...
vector<struct Model> readModels(){
    vector<char> words = {'1','2','3','4','5','6','7','8', 'a', 'b', 'c', 'd', 'e', 'f', 'g', 'h'};
    vector<struct Model> digitModels;

    for(char word : words){
        string file_name = base_path + "\\Models\\Model_" + word + ".txt";
        ifstream ifs(file_name);
        struct Model model;
...
char predictDigit(string file_name, vector<vector<ld>> codebook, vector<struct Model> digitModels){
    ...
    for(int i=0; i<n_words; ++i){
        ...
        cout << "Probability of " << words[i] << " is: " << curProb << endl;
    }
...
char recognizeWord(){
    ...
    string path = ".\\";
    string file_name = path + "Sample.txt";
    system("Recording_Module.exe 3 Sample.wav Sample.txt");
    char predicted = predictDigit(file_name, codebook, digitModels);
    cout << "The predicted word is: " << predicted << endl << endl;
    return predicted;
}
